...
def consigne_redaction(facteurs: tuple[str, ...]) -> str:
    ...
    return (
        ...
        f"{bloc}"
        "Règles : VOUVOIE toujours le producteur (« vous »), jamais « tu » — c'est la "
        "règle du service, et un registre qui change d'un écran à l'autre se remarque "
        "immédiatement. Ne nomme aucune maladie et aucun ravageur ; ne propose aucun "
        "produit ni aucune dose. Tu peux conseiller des gestes sans produit (récolte "
        "sanitaire, évacuation des cabosses atteintes, élagage, aération). Termine en "
        "invitant le producteur à montrer ces photos à son agent ANADER pour "
        "confirmation.\n"
