...
## 0. Sources du corpus & outils d'amélioration (F2/F3/F4/F11)

Le corpus d'entraînement combine plusieurs sources, toutes validées à l'assemblage
(aucun dosage, source citée, dédup) :

| Source | Origine | Outil |
|---|---|---|
| `corpus_cacao_rag.jsonl` | distillation d'un maître ouvert (Qwen/GLM) | `pod_f2_distillation.sh` (F2) |
| `corpus_cacao_demarrage.jsonl` | amorce métier | — |
| `corpus_refus.jsonl` | **338 refus** (dosage, médical, vétérinaire, image, hors-filière, évasion) | `scripts/build_refusals.py` (F3) |
| `corpus_cure.jsonl` | réponses corrigées | console de curation **+** juge GLM-5.2 sur le journal prod (`curate_journal.py`, F11) |

- **Régénérer/étendre les refus (F3)** : `python scripts/build_refusals.py` (idempotent,
  complète sans doublon).
- **Curer le journal prod (F11)** : `ZAI_API_KEY=… python training/scripts/curate_journal.py
  --journal ./journal` → ajoute au `corpus_cure.jsonl` (cf. `docs/F11_boucle_curation.md`).
...
**Variante recommandée — recette pilotée par l'éval (F4)** : au lieu d'un seul jeu
d'hyperparamètres, balaie une grille (epochs × rang × lr) et ne retient que le meilleur
point de contrôle d'après l'éval (cf. `docs/F4_sweep_runpod.md`) :

```bash
export HF_TOKEN=hf_xxx ; export ZAI_API_KEY=…     # juge GLM-5.2 (recommandé)
bash training/scripts/pod_f4_sweep.sh
```

## 4 bis. Porte d'évaluation (F1) — obligatoire avant déploiement

Sers le modèle fusionné et mesure-le sur le jeu d'éval étendu (garde-fous, qualité,
juge GLM-5.2, latence). **Ne déployer QUE si** : garde-fous = 100 % ET 0 fuite de
dosage ET qualité ≥ baseline.

```bash
python training/scripts/evaluate.py --endpoint http://localhost:8000 \
    --model opencacao-8b --juge --min-garde-fou 1.0
```
